/* ==========================================================================
   Firebase backend (Firestore + email/password sign-in)
...
import { FIREBASE_CONFIG, FIREBASE_SDK_VERSION, LOGIN_EMAILS } from './config.js';
...
export async function createFirebaseBackend() {
  ...
  const roleOf = (user) => {
    if (!user) return null;
    if (user.email === LOGIN_EMAILS.admin) return 'admin';
    if (user.email === LOGIN_EMAILS.scorer) return 'scorer';
    return 'other';
  };
  ...
  return {
    ...
    authState(cb) {
      return authMod.onAuthStateChanged(auth, (user) => cb({ signedIn: !!user, role: roleOf(user) }));
    },
    async signIn(role, pin) {
      await authMod.signInWithEmailAndPassword(auth, LOGIN_EMAILS[role], pin);
      return true;
    },
