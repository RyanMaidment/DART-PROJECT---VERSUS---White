...
export async function createStore() {
  ...
  return {
    ...
    /** { "1": "The Honey Badgers", "4": "" } — an empty string removes a team's custom name */
    saveTeamNames: (map) => be.setDoc('league/config', { teamNames: map }, { merge: true }),
    ...
    clearMatch(nightId, matchId) {
      return be.updateDoc(matchPath(nightId, matchId), { legs: {}, status: 'pending', readyA: false, readyB: false, updatedAt: Date.now() });
    },
    ...
    async saveWeekly(nightId) {
      ...
      const rows = weeklyRows({ night, matches, players, rules, teamNames: config && config.teamNames });
